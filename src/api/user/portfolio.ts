...
export const portfolioApi = {
  ...
  getPortfolios(clientId: number) {
    return request.get(`/user/clients/${clientId}/portfolios`)
  },

  // 获取 Portfolio 详情
  getPortfolioById(clientId: number, portfolioId: number) {
    return request.get(`/user/clients/${clientId}/portfolios/${portfolioId}`)
  },

  // 创建 Portfolio
  createPortfolio(data: CreatePortfolioParams) {
    return request.post(`/user/clients/${data.clientId}/portfolios`, {
      bank: data.bank,
  ...
  updatePortfolio(clientId: number, portfolioId: number, data: UpdatePortfolioParams) {
    return request.put(`/user/clients/${clientId}/portfolios/${portfolioId}`, {
      bank: data.bank,
      bookingCentre: data.bookingCentre,
      portfolioNo: data.portfolioNo
    })
  ...
  deletePortfolio(clientId: number, portfolioId: number) {
    return request.delete(`/user/clients/${clientId}/portfolios/${portfolioId}`)
  }
}
